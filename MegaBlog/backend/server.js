const express = require("express");
const cors = require("cors");
const sdk = require("node-appwrite");
const { Client } = require("appwrite");

const app = express();
const PORT = 3001; // Use a port of your choice

app.use(cors());
app.use(express.json());

const appwriteClient = new sdk.Client();
appwriteClient
  .setEndpoint("https://cloud.appwrite.io/v1")
  .setProject("657c886c4b12d971d777")
  .setKey(
    ""
  );

// Example route to get blog posts
app.get("/api/posts", async (req, res) => {
  try {
    const posts = await appwriteClient.listDocuments("657c89409e291365ef8e"); // replace with your actual collection ID
    res.json(posts.documents);
  } catch (error) {
    console.error("Error fetching blog posts:", error);
    res.status(500).json({ error: "Internal Server Error" });
  }
});

app.listen(PORT, () => {
  console.log(`Appwrite server is running on http://localhost:${PORT}`);
});
